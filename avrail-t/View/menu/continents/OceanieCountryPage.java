 ...
 * @param args
 */
public class OceanieCountryPage extends JFrame {
    GameBoard gameBoard;
    Controller controller;

    public OceanieCountryPage(Controller controller, GameBoard gameBoard) {
        ...
        setTitle("Choix du pays d'Oceanie");
        ...
        JLabel labelAustralie = new JLabel("Australie");
        labelAustralie.setBounds(480, 150, 250, 250);
        ImageIcon icon = new ImageIcon("Resources/backgroundMaps/villes-oceanie/mapAustralie.jpeg");
        img = icon.getImage();
        newImg = img.getScaledInstance(270, 270, Image.SCALE_SMOOTH);
        JButton Australie = new JButton(new ImageIcon(newImg));
        Australie.setBounds(400, 300, 250, 250);

        Border border = BorderFactory.createMatteBorder(3, 3, 3, 3, Color.BLUE);
        Australie.setBorder(border);

        Font boldFont = new Font("Courier New", Font.BOLD, 20);
        labelAustralie.setFont(boldFont);
        labelAustralie.setForeground(Color.WHITE);
        label.add(labelAustralie);
        label.add(Australie);
